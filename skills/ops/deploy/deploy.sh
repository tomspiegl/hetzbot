...
run "rsync -az --delete '$HETZBOT_ROOT/skills/' $host:/opt/hetzbot/skills/"
run "ssh $host 'find /opt/hetzbot/skills -type f -name \"*.sh\" -exec chmod +x {} +'"

for svc in $services; do
  run "rsync -az --delete '$HETZBOT_FLEET_ROOT/services/$svc/' $host:/opt/hetzbot/services/$svc/"
done
...
run "ssh $host 'sudo /opt/hetzbot/skills/infra/docker/install.sh'"
run "ssh $host 'sudo /opt/hetzbot/skills/infra/restic/install.sh'"
...
for skill_dir in "$HETZBOT_ROOT"/skills/infra/*/; do
  ...
  run "ssh $host 'cd /opt/hetzbot/skills/infra/$skill && sudo docker compose up -d --wait'"
done
...
for svc in $services; do
  ...
  run "ssh $host 'sudo /opt/hetzbot/skills/ops/deploy/install-service.sh $svc \"$source_url\"'"
done

# --- 6. Caddy (public hosts only) — install + assemble ---
if [ "$public" = "true" ]; then
  log "ensuring caddy installed + reassembling /etc/caddy/Caddyfile"
  run "ssh $host 'sudo /opt/hetzbot/skills/infra/caddy/install.sh'"
  run "ssh $host 'sudo /opt/hetzbot/skills/infra/caddy/assemble.sh'"
fi
